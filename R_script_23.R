...
##### import vars
in_vars <- read.table("all_tumor_all_genes_hpv_snps_INCLUSIVE_with_TUMOR_ID.vcf", header = FALSE, comment.char = "#")
...
in_vars <-subset(in_vars, in_vars$study_id %in% NOT_UNCseq_Sample_Data_hpv16_POS$sample)
...
new_vars$depth <- as.numeric(new_vars$depth)

clonal_vars <- subset(new_vars, new_vars$vaf >= 90)
clonal_vars <- subset(clonal_vars, clonal_vars$depth > 1)

sample_dictionary <- NOT_UNCseq_Sample_Data_hpv16_POS$study_id
names(sample_dictionary) <- NOT_UNCseq_Sample_Data_hpv16_POS$sample

clonal_vars$study_id <- sample_dictionary[clonal_vars$study_id]
...
samples <- names(test)
for(i in samples){
  coverage <- read.table(file = paste("/home/parke/ssd2c/UNC_OP_RNA_cohort/hpv16_coverage/", i, ".hpv16.coverage.txt", sep = "" ))
  ## CAN USE THE LINE BELOW TO INCREASE YOUR MINIMUM COVERAGE ABOVE 1
  #coverage <- subset(coverage, coverage$V3 > 1)
  print(100*(7906 - length(coverage$V3))/7906)
  for(ii in 1:7906){
    if(!(ii %in% coverage$V2 )){
      #print("changes")
      test_x[[i]][ii] <- "N"
    }
  }
...
names(test_x) <- sample_dictionary[names(test_x)]

sequences <- c()
for(i in names(test_x)){
  sequences <- c(sequences, str_c(test_x[[i]], collapse = ""))
}
...
seq_names <- c(names(test_x), "A1", "A2", "A3", "A4", "B1","B2", "B3", "B4", "C1", "C2", "C3", "C4", "D1", "D2", "D3", "D4")


write.fasta(out_seq, seq_names, file.out = "reference_and_sample_HPV16_genotypes_RNA_DATA_NOT_UNCseq.fasta", open = "w", nbchar = 60, as.string = FALSE)
