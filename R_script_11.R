...
for(i in 1:length(test_names)){
  ...
  nearest_neighbor <- c(nearest_neighbor, holder$study_id[1])
}


prediction_data <- data.frame(study_id = test_names_short, prediction = predictions, nearest_neighbor = nearest_neighbor)
prediction_data_validate <- prediction_data
save(prediction_data_validate, file = "prediction_data_validate.RData")
